...
デジタル出力の場合、マイコンと直結すれば制御可能ですが、Serial , I2C , SPI の接続IFに応じてプログラムが必要です。BME260,BME680等、有名なセンサは制御用モジュール(ドライバと呼ばれます)が公開されています。センサ名で、例えば、　github  micropython driver BME280  のキーワードで検索すれば、ドライバを見つけられます。Copilotに質問しても回答が得られます。
