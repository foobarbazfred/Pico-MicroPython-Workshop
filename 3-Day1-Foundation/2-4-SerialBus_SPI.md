...
SPIバスの仕様上、マスタ・スレーブ間のデータの送受信は規定されていますが、データをどう扱うのかについては規定されていません。例えば、センサがレジスタを持つ場合、センサのレジスタに値を書き込むのか、センサのレジスタから値を読み込むのか、SPIバスの仕様では定義できません。このような使い方はデバイス各社がデータの扱いを独自に決めて運用しています。
