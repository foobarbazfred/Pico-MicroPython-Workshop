...
RP2350にはDA変換が搭載されていませんので、直接アナログ出力することができません。代替手段として、PWMを用いて、0、1(3.3V)以外の値（中間値）を表現することができます。厳密には、PWMのパルス幅の比率を使って変調して出力します(PWM : Pulse Width Modulation)。PWMで疑似的に表現された中間値をローパスフィルタ(LPF)（積分回路）に通すと、アナログ値が取り出せます。この講習では、LPFは使わず、そのままデバイスに投入します。
