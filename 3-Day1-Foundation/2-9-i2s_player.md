...
DACから音を出す場合、デジタル化された音データを作る必要があります。例えば、CDの音は非圧縮の、44.1KHz/16bitサンプリングのLRデータの並びです。WAVファイルは、任意のサンプリングレートで、16bitサンプリングの非圧縮データです。音データは高サンプリングレートによりデータが大きくなり、SDカード等の大容量ストレージがないと扱いが難しい問題があります。以下のサンプルは、プログラムで440Hz等のsin波、1波分を作り、DACに連続的に送信する例です。
