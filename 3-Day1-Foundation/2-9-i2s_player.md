...
### 音の再生
DACから音を出す場合、デジタル化された音データを作る必要があります。例えば、CDの音は非圧縮の、44.1KHz/16bitサンプリングのLRデータの並びです。WAVファイルは、サンプリングレート（16/24/48 kHz）で、16bitサンプリングの非圧縮データです(モノラル、ステレオ選択可)。音データは高サンプリングレートによりサイズが大きくなり、SDカード等の大容量ストレージがないと扱いが難しい問題があります。以下のサンプルは、プログラムで440Hz等のsin波、1波分を作り、DACに連続的に送信する例です。440Hzの場合、440回、１波分のデータ転送を繰り返すと１秒間、440Hzの音が再生できます。
