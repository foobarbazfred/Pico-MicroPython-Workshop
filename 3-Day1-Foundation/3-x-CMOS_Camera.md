# CMOSカメラの利用
マイコンでカメラを扱うにはカメラから高速で画像データを取り込む必要があります。カメラとの接続IFとして一般的にはUSBやパラレルがありますが、RP2+MicroPythonの組み合わせでは性能が間に合わずまともな画像取り込みは難しい問題があります。性能が不足して一般的な方法では画像取り込みが難しいマイコンにとって制御しやすいカメラとしてArducamが挙げられます。
Arducamの特徴は以下です。
- 画像用フレームバッファを内蔵
