...
SPIを用いてSDカードと接続することができます。SDカードにはSDモード（ネイティブモード）以外にSPIバスで接続できる、SPIモードがあります。SDモードで動作しているSDカードに対してSPIモードに切り替えて接続します。SPIを用いたマイコンとSDカードとのやり取りはSDカード用ドライバが提供されており、SDカード用ドライバを用いることで、数行のコードを書くだけでSDカードが利用可能になります。RP2用のMicroPythonではSDカード用ドライバが組み込まれていないのでGitHubから入手する必要があります。
